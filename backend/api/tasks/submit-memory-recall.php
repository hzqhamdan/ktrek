...
try {
    ...
    $query = "SELECT t.*, a.name as attraction_name, a.id as attraction_id, a.category
              ...
              WHERE t.id = :task_id AND t.type = 'memory_recall'";
    ...
    if ($stmt->rowCount() === 0) {
        $db->rollBack();
        Response::error("Memory Recall task not found", 404);
    }

    $task = $stmt->fetch(PDO::FETCH_ASSOC);
    $task_config = json_decode($task['task_config'], true);

    // 2. Get recall question (should be question 2) and check answer
    $query = "SELECT qq.id as question_id, qq.question_text, qq.question_order,
    ...
    // 3. Get all options for the recall question
    $query = "SELECT qo.id as option_id, qo.option_text, qo.is_correct
              FROM quiz_questions qq
              LEFT JOIN quiz_options qo ON qo.question_id = qq.id
              WHERE qq.task_id = :task_id AND qq.question_order = 2
    ...
    ], "Memory recall submitted successfully", 201);

} catch (PDOException $e) {
    $db->rollBack();
    error_log("Memory recall submission error: " . $e->getMessage());
    Response::serverError("Failed to submit memory recall");
}
?>
