...
const GradientBackground = () => (
    <>
        ...
        <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full">
            <defs>
                <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'rgb(var(--primary))', stopOpacity:0.55}} /><stop offset="100%" style={{stopColor: 'rgb(var(--chart-3))', stopOpacity:0.35}} /></linearGradient>
                <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'rgb(var(--chart-4))', stopOpacity:0.45}} /><stop offset="50%" style={{stopColor: 'rgb(var(--secondary))', stopOpacity:0.35}} /><stop offset="100%" style={{stopColor: 'rgb(var(--chart-1))', stopOpacity:0.4}} /></linearGradient>
                <radialGradient id="rev_grad3" cx="50%" cy="50%" r="50%"><stop offset="0%" style={{stopColor: 'rgb(var(--destructive))', stopOpacity:0.25}} /><stop offset="100%" style={{stopColor: 'rgb(var(--chart-5))', stopOpacity:0.18}} /></radialGradient>
            ...
            <g style={{ animation: 'float2 25s ease-in-out infinite' }}>
                ...
                <ellipse cx="50" cy="150" rx="180" ry="120" fill="rgb(var(--accent))" filter="url(#rev_blur2)" opacity="0.55"/>
            </g>
        </svg>
    </>
);
...
interface AuthComponentProps {
  ...
  // Optional early check: used on the email step to show "already registered" before moving forward.
  onCheckIdentifier?: (emailOrPhone: string) => Promise<{success: boolean; message?: string}>;
  onGoogleSignIn?: () => void;
}

export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "K-Trek", onRegister, onCheckIdentifier, onGoogleSignIn }: AuthComponentProps) => {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    // If onRegister callback is provided, call backend API
    if (onRegister) {
        try {
  ...
  const handleProgressStep = async () => {
    if (authStep === 'email') {
      ...
      // Early duplicate check (if provided) so the user gets feedback immediately.
      if (onCheckIdentifier) {
        try {
          const res = await onCheckIdentifier(email);
          if (!res.success) {
            setModalErrorMessage(res.message || "This email is already registered");
            setModalStatus('error');
            return;
          }
        } catch (err: any) {
          setModalErrorMessage(err?.message || "Unable to verify email right now");
          setModalStatus('error');
          return;
        }
      }
...
useEffect(() => {
  ...
  <button
    ...
    onClick={closeModal}
    aria-label="Close"
    className="auth-plain-btn absolute top-2 right-2 h-9 w-9 rounded-full bg-transparent hover:bg-white/10 transition flex items-center justify-center text-foreground/60 hover:text-foreground"
  >
    <X className="w-5 h-5" />
  </button>
)}
    ...
    <div className="bg-background min-h-screen w-screen flex flex-col">
        ...
        <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-[rgb(var(--secondary))]", "relative overflow-hidden")}>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-8">
                     <AnimatePresence>
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-10">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    ...
                                    <div className="glass-input-wrap w-full"><div className="glass-input bg-transparent !bg-transparent shadow-none !shadow-none border border-white/20 !border-white/20">
                                        <span className="glass-input-text-area opacity-10 pointer-events-none"></span>
                                        ...
                                        <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={handleKeyDown} className={cn("glass-auth-input relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none transition-[padding-right] duration-300 ease-in-out delay-300", isEmailValid && authStep === 'email' ? "pr-2" : "pr-0")} />
                                        <div className={cn("relative z-10 flex-shrink-0 transition-all duration-300 ease-in-out", isEmailValid && authStep === 'email' ? "w-10 pr-1" : "w-0")}>
                                          <button
                                            type="button"
                                            onClick={handleProgressStep}
                                            aria-label="Continue with email"
                                            className="auth-plain-btn h-10 w-10 rounded-full bg-transparent backdrop-blur-sm border border-white/20 text-foreground/60 hover:text-foreground/90 hover:bg-white/5 transition flex items-center justify-center"
                                          >
                            ...
                            {authStep === "email" && (
                              <BlurFade delay={0.25 * 6} className="w-full">
                            ...
                            <AnimatePresence>
                                {authStep === "password" && (
                                  <div style={{ marginTop: "1.5rem" }}>
                                    <BlurFade key="password-field" className="w-full">
                                      <div className="relative w-full">
                                        ...
                                        <div className="glass-input-wrap w-full"><div className="glass-input bg-transparent !bg-transparent shadow-none !shadow-none border border-white/20 !border-white/20">
                                            ...
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="glass-auth-input relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                                            <div className={cn("relative z-10 flex-shrink-0 transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0")}>
                                              <button
                                                type="button"
                                                onClick={handleProgressStep}
                                    ...
                                    <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} style={{ marginTop: "1.75rem" }} className="auth-plain-btn flex items-center gap-2 text-sm text-foreground/40 hover:text-foreground/80 transition-colors bg-transparent backdrop-blur-sm px-3 py-1 rounded-full"><ArrowLeft className="w-4 h-4" /> Go back</button></BlurFade>
                                    </BlurFade>
                                  </div>
                                )}
                            </AnimatePresence>
                    ...
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                ...
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input bg-transparent !bg-transparent shadow-none !shadow-none border border-white/20 !border-white/20">
                                    <span className="glass-input-text-area opacity-10 pointer-events-none"></span>
                                    <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                    ...
                                    <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="glass-auth-input relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                                    <div className={cn("relative z-10 flex-shrink-0 transition-all duration-300 ease-in-out", isConfirmPasswordValid ? "w-10 pr-1" : "w-0")}>
                                      <button
                                        type="submit"
