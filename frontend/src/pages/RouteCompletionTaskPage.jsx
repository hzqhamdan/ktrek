...
import RouteCompletionTask from "../components/tasks/RouteCompletionTask";
...
const RouteCompletionTaskPage = () => {
  ...
  const handleComplete = (responseData) => {
    showToast(responseData.is_correct ? "Route completed!" : "Task completed", "success");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background-light">
      {currentTierUnlock && (
        <TierUnlockModal isOpen={true} onClose={() => { clearTierUnlock(); if (rewardData) setShowRewardModal(true); }}
          tier={currentTierUnlock.tier} category={currentTierUnlock.category}
          epEarned={currentTierUnlock.epEarned} completionPercentage={currentTierUnlock.completionPercentage}
        />
      )}
      <RewardNotificationModal rewards={rewardData} isOpen={showRewardModal && !currentTierUnlock}
        onClose={() => { setShowRewardModal(false); setRewardData(null); navigate("/dashboard/progress"); }}
      />
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <RouteCompletionTask task={task} onComplete={handleComplete} />
      </div>
    </div>
  );
};

export default RouteCompletionTaskPage;
